<template>
  <div class="order-list-view">
    <!-- 页面标题 -->
    <x-header :left-options="{backText:''}">待发货</x-header>
    <div class="content-view-scroller">
      ...
      <router-link to="/order_info" class="order-item" v-for="(item,index) in 4" :key="index">
        ...
        <div class="text-right">
          <span class="btn this-btn" @click="remind">提醒发货</span>
        </div>
      </router-link>
      <!-- 订单项目 -->
      <router-link to="/order_info" class="order-item" v-for="(item,index) in 5" :key="index">
        ...
        <!-- 订单操作 -->
        <div class="text-right">
          <span class="btn next-btn" @click="remind">提醒发货</span>
        </div>
      </router-link>
    </div>
  </div>
  ...
  export default{
    ...
    methods: {
      remind () {
        ...
        this.$vux.alert.show({content: '提醒卖家发货'})
      }
    }
  }
</script>
