<template>
  <div class="order-list-view">
    <!-- 页面标题 -->
    <x-header :left-options="{backText:''}">配送中</x-header>
    <div class="content-view-scroller">
      ...
      <router-link to="/order_info" class="order-item" v-for="(item,index) in 4" :key="index">
        ...
        <div class="text-right">
          <span class="btn this-btn" @click="contact">联系店家</span>
        </div>
      </router-link>
      <!-- 订单项目 -->
      <router-link to="/order_info" class="order-item" v-for="(item,index) in 5" :key="index">
        ...
        <!-- 订单操作 -->
        <div class="text-right">
          <span class="btn next-btn" @click="contact">联系店家</span>
        </div>
      </router-link>
    </div>
  </div>
  ...
  export default{
    ...
    methods: {
      contact () {
        ...
        this.$vux.alert.show({content: '联系店家'})
      }
    }
  }
</script>
